    ...
    <thead>
        <tr>
            ...
            if ($distinctDatesResult) {
              $distinctRegistrationQuery = "SELECT DISTINCT employeeRegistrationNumber FROM tblattendance WHERE tmonth='$tmonthCode' AND tweek='$tweekCode' ORDER BY employeeRegistrationNumber ASC";
...
if ($distinctRegistrationResult) {
    while ($registrationRow = mysqli_fetch_assoc($distinctRegistrationResult)) {
        $registrationNumber = $registrationRow['employeeRegistrationNumber'];
        echo "<tr>";
        echo "<td>" . $registrationNumber . "</td>";
        $attendanceQuery = "SELECT dateMarked, attendanceStatus FROM tblattendance WHERE employeeRegistrationNumber = '$registrationNumber'";
...
function updateTable(){
    ...
    var headerContent = '<p style="font-weight:700;"> Attendance for : ' + tmonthCode + ' Week name : ' + tweekCode + ' On: ' + formattedDate + '</p>';
